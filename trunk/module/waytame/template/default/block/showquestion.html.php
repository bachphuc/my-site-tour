...
{/foreach}
{else}
<p style="color: #666;">{phrase var='waytame.no_questions_found'}</p>
{/if}
...
<div style="text-align: center;pading:5px;">
    <span val="0" style="display: none;" class="waytame_show_less_question" onclick="$.ajaxCall('waytame.showMoreQuestion','{if Phpfox::getLib('request')->get('view') == 'expire'}view=expire&{/if}user_id={$aQuestions.0.user_id}&page='+$(this).attr('val'))"></span>
    <span val="1" class="waytame_show_more_question" onclick="$.ajaxCall('waytame.showMoreQuestion','{if Phpfox::getLib('request')->get('view') == 'expire'}view=expire&{/if}user_id={$aQuestions.0.user_id}&page='+$(this).attr('val'))"></span>
</div>
{/if}
