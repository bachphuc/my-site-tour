    ...
    */
    class FollowedPost_Component_Controller_Index extends Phpfox_Component {
        ...
        public function process()
        {
            ...
            //            d($aStrongFeed[0]);die();
           /* for($i =0; $i <count($aFollowed);$i++)
            {
                if(isset($aFollowed[$i]['comments']))
                {
                    $lenght = count($aFollowed[$i]['comments']);
                    for($j =0; $j <$lenght;$j++)
                    {
                        if(isset($aFollowed[$i]['comments'][$j]['comment_id']))
                        {
                            $idComment = $aFollowed[$i]['comments'][$j]['comment_id'];
                            $isCommentSB = Phpfox::getService('followedpost')->checkCommentVisible($idComment);
                            if(!$isCommentSB)
                            {
                                unset($aFollowed[$i]['comments'][$j]);
                                $lenght++;
                            }
                        }
